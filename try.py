...
from model.products import insert_product, get_products_data, get_products_count, delete_products_id, update_product, get_product_id
...
def login_func():
    ...
    refresh_token = jwt.encode({
        "email": email,
        "role": db_data.get("role"),   # add role
...
def addProducts(payload):
    ...
    if not all([name, description, price, category, stock]):
        return jsonify({"error": "send complete data"}), 400
...
@app.route("/getProducts/<product_id>", methods=["GET"])
@token_required
@admin_required
def getProductsId(payload, product_id):

    try:
        data = get_product_id(product_id)

        if not data:
            return jsonify({"error": "product not found, 404"}), 409
        
        result = {
            "_id": str(data["_id"]),  # ObjectId -> str
            "name": data["name"],
            "category": data.get("category") or data.get("cateogry"),
            "description": data.get("description"),
...
def editCart():
    ...
    try:
        ...
        # Find user's cart
        cart = find_user_cart({"user_id": ObjectId(user_id)})
        if not cart:
